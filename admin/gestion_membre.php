...
$content .= '<h3>Affichage des '.$r->rowCount().' membres :</h3>';
...
while( $membre = $r->fetch(PDO::FETCH_ASSOC) ){
  ...
  $content .= '
  <td>
    <a href="?action=suppression&id_membre='.$membre['id_membre'].'"  onClick="return(confirm(\'Supprimer le compte de '.$membre['prenom'].' '.$membre['nom'].'?\') )" >suppr</a>
  </td>';
  $content .= '</tr>';
}
$content .= '</table>';
